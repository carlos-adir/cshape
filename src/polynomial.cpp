...
Polynomial<I, O>::Polynomial(const Polynomial &other)
    ...
    for (uint1 i = 0; i <= odegree; i++) (*this)[i] = other[i];
};
...
template <typename I, typename O>
Polynomial<I, O> Polynomial<I, O>::operator*(
    const Polynomial<I, O> &other) const {
...
template <typename I, typename O>
Polynomial<I, O> Polynomial<I, O>::operator+(const O other) const {
...
template <typename I, typename O>
Polynomial<I, O> Polynomial<I, O>::operator-() const {
    Polynomial<I, O> newfunc;
    newfunc = *this;
    newfunc *= -1;
    return newfunc;
...
O Polynomial<I, O>::eval(const I node, const uint1 deriv) const {
    ...
    for (uint1 i = degree; i > deriv; i--) {
        result *= node;
        deriv_coef = 1;
        for (uint1 j = 0; j < deriv; j++) deriv_coef *= i - j;
        result += deriv_coef * this->internal[i];
    }
    result *= node;
    deriv_coef = 1;
    for (uint1 j = 0; j < deriv; j++) deriv_coef *= deriv - j;
